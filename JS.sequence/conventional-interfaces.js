...
import { node, append, make, reduce, toString as htmlToString }
...
export const extractHeaders = (elements) => {
    ...
    const result = map(element => node('p', value(element)), filtred);
    return result;
};

export const wordsCount = (tag, element, elements) => {
    const filtred = filter(list => is(tag, list), elements);
    const text = htmlToString(filtred);
    const items = wc(element, text);
    return items;
};
// END
